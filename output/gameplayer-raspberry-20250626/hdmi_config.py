...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("hdmi_config.log"), logging.StreamHandler()],
)
...
class HDMIConfigurator(BaseInstaller):
    ...
    def parse_config_line(self, line: str) -> Optional[Dict[str, str]]:
        ...
        if "=" in line:
            ...
            return {"key": key.strip(), "value": value.strip(), "line": line}
    ...
    def validate_config(self) -> bool:
        """验证配置是否有效"""
        logger.info("验证HDMI配置...")

        # 检查关键配置项
        required_configs = ["hdmi_group", "hdmi_mode", "gpu_mem"]
        lines = self.read_config()

        for config in required_configs:
            line_index = self.find_config_line(lines, config)
            if line_index is None:
                logger.warning(f"缺少配置项: {config}")
                continue

            parsed = self.parse_config_line(lines[line_index])
            if not parsed:
                logger.warning(f"配置项格式错误: {config}")
                continue

            logger.info(f"✓ {config}={parsed['value']}")
