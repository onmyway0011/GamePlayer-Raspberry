...
try:
    from systems.retropie.core.logger_config import get_logger
...
# 配置日志
logger = get_logger("rom_downloader")
...
class ROMDownloader:
    ...
    def connect_sftp(self) -> Optional[paramiko.SSHClient]:
        ...
        try:
            ...
            # 使用密钥文件或密码
            if pi_config.get("key_file") and Path(
                    pi_config["key_file"]).exists():
                ssh.connect(
