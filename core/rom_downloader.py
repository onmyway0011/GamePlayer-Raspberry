        ...
        try:
            ...
            for filename, file_info in files.items():
                if filename.lower().endswith('.zip'):
                    download_url = f"{self.config['rom_sources']['archive_org']['base_url']}/download/{identifier}/{filename}"
    ...
    def extract_roms(self, zip_path: Path) -> List[Path]:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                rom_files_in_zip = [f for f in file_list if any(f.endswith(ext) for ext in rom_extensions)]
    ...
    def run(self, search_query: str = "nes 100 in 1"):
        ...
        if not zip_path:
            ...
            for i, result in enumerate(results[:5]):
                print(f"{i+1}. {result['title']} (下载次数: {result['downloads']})")
...
def main():
    ...
    if args.upload_only:
        ...
        if extract_dir.exists():
            rom_files = list(extract_dir.glob("*.nes")) + list(extract_dir.glob("*.NES"))
            if rom_files:
