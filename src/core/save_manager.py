...
class SaveManager:
    ...
    def save_game(self, rom_path: str, game_state: Dict, slot: int = 0):
        """保存游戏进度"""
        try:
    ...
    def delete_save(self, rom_path: str, slot: int = 0):
        """删除存档"""
        try:
            game_id = self.get_game_id(rom_path)
            save_path = self.get_save_path(game_id, slot)
    ...
    def start_auto_save(self, rom_path: str, get_game_state_func):
        """启动自动保存"""
    ...
    def sync_to_cloud(self, game_id: str, slot: int, save_data: Dict):
        """同步存档到云端"""
