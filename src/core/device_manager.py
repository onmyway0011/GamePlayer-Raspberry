...
class DeviceManager:
    ...
    def _check_bluetooth_connection_linux(self, mac_address: str):
        """检查Linux蓝牙设备连接状态"""
        try:
            result = subprocess.run([
    ...
    def connect_bluetooth_device(self, device_info: Dict):
        """连接蓝牙设备"""
        try:
    ...
    def _connect_bluetooth_linux(self, device_info: Dict):
        """Linux蓝牙设备连接"""
        try:
    ...
    def _connect_bluetooth_macos(self, device_info: Dict):
        """macOS蓝牙设备连接"""
        # macOS的蓝牙连接通常通过系统设置进行
        print(f"ℹ️ macOS蓝牙设备请通过系统设置连接: {device_info['name']}")
        return True

    def _connect_bluetooth_windows(self, device_info: Dict):
        """Windows蓝牙设备连接"""
