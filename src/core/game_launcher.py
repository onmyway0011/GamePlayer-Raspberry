...
class GameLauncher:
    ...
    def install_emulator(self, system: str) -> bool:
        """安装模拟器"""
        if system not in self.emulator_configs:
            return False
    ...
    def apply_cheat_codes(self, system: str, game_id: str, cheats: List[Dict]) -> bool:
        """应用金手指代码"""
        try:
            # 创建金手指文件
    ...
    def load_save_state(self, system: str, game_id: str, slot: int = 1) -> bool:
        """加载存档"""
        try:
