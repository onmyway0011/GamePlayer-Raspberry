...
check_requirements() {
    ...
    local required_tools=("sudo" "docker" "python3" "pip3" "git" "curl" "unzip")
    local missing_tools=()
...
prepare_roms() {
    ...
    if [ -f "src/scripts/rom_downloader.py" ]; then
        log_info "下载合法ROM文件..."
        python3 src/scripts/rom_downloader.py --all --output data/roms/nes/ || true
    fi
    
    # 运行ROM管理器
    if [ -f "src/scripts/rom_manager.py" ]; then
        log_info "管理ROM文件..."
        python3 src/scripts/rom_manager.py --verify --clean || true
    fi
...
build_docker_images() {
    ...
    # 构建树莓派模拟镜像
    log_info "构建树莓派模拟镜像..."
    if [ -f "build/docker/Dockerfile.raspberry-sim" ]; then
        docker build -f build/docker/Dockerfile.raspberry-sim -t gameplayer-raspberry:raspberry-sim . || true
    fi
    
    # 构建GUI镜像
    log_info "构建GUI镜像..."
    if [ -f "build/docker/Dockerfile.gui" ]; then
        docker build -f build/docker/Dockerfile.gui -t gameplayer-raspberry:gui . || true
    fi
    
    # 构建Web管理镜像
    log_info "构建Web管理镜像..."
    if [ -f "build/docker/Dockerfile.web-manager" ]; then
        docker build -f build/docker/Dockerfile.web-manager -t gameplayer-raspberry:web-manager . || true
    fi
